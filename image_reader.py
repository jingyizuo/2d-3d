...
def ImageReader(file_name, picture_path, label_path, picture_format = ".tif", label_format = ".tif", size = 256):
    ...
    picture_ = cv2.imread(picture_name,0) #读取图片 
    picture = np.expand_dims(np.array(picture_).astype(np.uint8), axis = 2) #填充维度 
    label_ = cv2.imread(label_name,0) #读取标签 
    label = np.expand_dims(np.array(label_).astype(np.uint8), axis = 2) #填充维度
    ...
    picture_resize = picture / 127.5 - 1. #归一化图片
    #label_resize_t = cv2.resize(label, (size, size)) #调整标签的尺寸，改变成网络输入的大小
    label_resize = label / 127.5 - 1. #归一化标签
